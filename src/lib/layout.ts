/**
 * layout.ts — 생산 계획을 격자 위의 배치 후보로 바꾸는 레거시 휴리스틱.
 *
 ...
 *
 * 기존 Claude 조사 근거는 폐기했다. 실제 게임 데이터와 공정 설계 근거를 다시 검증하기 전까지 이 모듈의
 * 치수·분배·블루프린트 가정은 제품 규칙으로 승격하지 않는다. PRODUCT의 자동 배치 제외 결정에 따라
 * 사용자 배치를 대신하는 기능으로 노출해서도 안 된다.
 */
