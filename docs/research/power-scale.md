...
## 추가 확인 — 고체 바이오 연료로 갈아타는 시점 (2026-08-20)

게임 데이터 계산값(바이오매스 연소기 30 MW 기준):

| 연료 | 개당 열량 | 연소기 1대 소요 | 원료 바이오매스 환산 |
|---|---|---|---|
| 바이오매스 | 180 MJ | 10개/분 | 10개/분 |
| **고체 바이오 연료** | **450 MJ** | **4개/분** | **8개/분** |

- **원료 20% 절감.** 바이오매스 2개(360 MJ)가 고체 바이오 연료 1개(450 MJ)가 되므로 가공에서 열량이 늘어난다.
- **벨트 한 칸당 열량 2.5배.** 같은 벨트 한 줄로 훨씬 많은 연소기를 감당한다.
- 제작기 한 대(고체 바이오 연료 60개/분)면 연소기 **15대분**이다.

**해금**: 티어 2 마일스톤 「장애물 제거」(`Recipe_Biofuel_C`). 허브 6에서 열리는 것은
바이오매스 레시피와 연소기까지이고 고체 바이오 연료는 한 티어 뒤다 — 가이드에서 순서를 섞지 않는다.
