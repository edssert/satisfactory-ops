...
**좌표가 있는 동굴 데이터는 따로 구했다** — §8.2 의 `caves.json` (84개 연결 동굴계, 이름 있는 것 17개).
이 데이터로 626개 노드 중 **28개가 동굴 안**임을 계산했다.
다만 위키의 이름(Stone Arch Ravine 등)과 이 데이터의 이름(Grass Fields Cave 3 등)이 서로 달라
**두 목록을 이름으로 대응시키지는 못했다.**

---

## 7. S.A.M. 노드 19개 — 좌표와 접근 방해 요소
...
2. **전투 난이도** — 스포너까지의 거리가 아니라 **등급으로 가중**해야 한다. 직접 세어 본 값:

   | 기준 | 100 m 안 | 50 m 안 |
   |---|---|---|
   | 아무 스포너 (2,826개) | 576/626 = **92.0%** | 418/626 = 66.8% |
   | 강한 것만 (핵돼지·알파·엘리트, 749개) | 256/626 = **40.9%** | 126/626 = 20.1% |

   전체를 세면 92%가 걸려서 **변별력이 없다.** 핵돼지·알파·엘리트만 세야 신호가 된다.
