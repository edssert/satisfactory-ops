...
## 추가 확인 — 파워 슬러그를 소머슬룹으로 뻥튀기 (2026-08-20)

게임 데이터로 확정한 사실:

| 항목 | 값 | 근거 |
|---|---|---|
| 슬러그 → 동력 조각 변환 기계 | **제작기** | `Recipe_PowerCrystalShard_1~3_C` 의 `producedIn` |
| 제작기의 소머슬룹 슬롯 | **1개** | `Build_ConstructorMk1_C.somersloopSlots = 1` |
| 소머슬룹 1개의 효과 | 산출 **2배** (전력은 제곱 → 4배) | 증폭 지수 2.0 |
...
**따라서 소머슬룹을 얻기 전에 슬러그를 변환하면 절반을 버리는 것이다.**
슬러그는 맵에 고정 개수로 놓여 있어 다시 자라지 않는다. 초반 가이드에 반드시 들어가야 할 경고다.

전력 비용은 제작기 4 MW → 16 MW 로 오르지만, 슬러그 변환은 상시 가동할 일이 아니라 무시해도 된다.

### 바로잡음
앞서 "슬러그로 소머슬룹을 만든다"로 잘못 읽었다. 그런 레시피는 없다 —
소머슬룹은 탐험으로만 얻고, 슬러그와 교환되지 않는다. 요점은 **소머슬룹을 슬러그 변환 제작기에
꽂아 산출을 두 배로 만드는 것**이다.

### 소머슬룹을 어디에 먼저 쓸 것인가
슬러그가 유한하므로 **슬러그 변환이 첫 번째 용도로 유력하다** — 소머슬룹 하나가
남은 슬러그 전부의 값어치를 두 배로 만든다. 그다음이 후반 고가 부품 증폭이다.
초반 부품을 두 배로 뽑아 봐야 원자재만 두 배로 든다.

기계별 소머슬룹 슬롯: 제작기 1 · 조립기 2 · 주조소 2 · 정제소 2 · 제조기 4 · 혼합기 4 ·
입자 가속기 4 · 변환기 2 · 양자 인코더 4.
