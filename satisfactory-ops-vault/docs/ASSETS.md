...
표현한다. Anders의 직선·회전·방향·끝단·분기·토대 역할은 카탈로그 탑뷰 도구로 전부 노출하고, 높이는
이미지 길이가 아니라 Z 값·라벨·레이어 가림 순서로 표현한다.

도감 아이소메트릭은 AI 생성·보정 이미지를 제품 자산으로 사용하지 않는다. 설비는 설치본 서술자의
`mSmallIcon` 원본 또는 게임 메시·재질 그래프에서 온 렌더만 사용하고, 토대 배치 예시는
