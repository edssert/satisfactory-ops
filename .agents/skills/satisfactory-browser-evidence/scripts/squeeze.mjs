 ...
 * 사용:
 *   npm run build
 *   node .agents/skills/satisfactory-browser-evidence/scripts/squeeze.mjs guide            # 기본 폭 전부
 *   node .agents/skills/satisfactory-browser-evidence/scripts/squeeze.mjs guide --w=390
 *   node .agents/skills/satisfactory-browser-evidence/scripts/squeeze.mjs "" --min=64      # 첫 화면, 임계치 바꿔서
 *
...
for (const W of WIDTHS) {
  ...
  const found = await page.evaluate((MIN_W) => {
    ...
    for (const el of document.querySelectorAll('body *')) {
      ...
      const wrapped = r.height > lineHeight * 1.55;
      if (inTrack && wrapped && t.length >= 4 && r.width > 0 && r.width < MIN_W) {
        let widest = 0;
